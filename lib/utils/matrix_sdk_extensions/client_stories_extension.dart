...
extension ClientStoriesExtension on Client {
  ...
  List<User> get contacts => rooms
      ...
      .map(
        (room) => room.unsafeGetUserFromMemoryOrFallback(room.directChatMatrixID!),
      )
      .toList();

  List<Room> get storiesRooms => rooms.where((room) => room.isStoryRoom).toList();
...
extension StoryRoom on Room {
  bool get isStoryRoom =>
      getState(EventTypes.RoomCreate)?.content.tryGet<String>('type') == ClientStoriesExtension.storiesRoomType;
}
