...
class _ChannelMoblePageState extends State<ChannelMoblePage> {
  ...
  bool __runImInit({AppCubit? cbt}) {
    final im = cbt ?? this.im;
    return !(im!.current == null || im.currentState == null);
  }

  __onImInit({AppCubit? cbt}) {
    final im = cbt ?? this.im;
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<AppCubit, AppState>(
        listenWhen: (p, c) => __runImInit(),
        listener: (c, s) => __onImInit(),
        child: Scaffold(
          appBar: AppBar(
            elevation: 0,
